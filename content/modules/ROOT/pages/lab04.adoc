...
The contents of the `idms-oc-mirror.yaml` and `itms-oc-mirror.yaml` __result files__ must be added to the `install-config.yaml` __answer file__.
...
[.highside,source,bash,role=execute,subs="attributes"]
----
if (test -e /mnt/high-side-data/working-dir/cluster-resources/idms-oc-mirror.yaml)
  then
     echo -e "\n\n Looks good, go ahead! \n\n"
   else
...
Look for a directory called `working-dir` on your [.highside]#highside# system.
Or ask a lab assistant for help.
--

When the check command says that everything looks good, you can add the `imageDigestMirrors:` and `imageTagMirrors:` data to your `install-config.yaml` with the following command.

[.highside,source,bash,role=execute]
----
cat << EOF >> /mnt/high-side-data/install-config.yaml
imageContentSources:
$(grep "mirrors:" -A 2 --no-group-separator /mnt/high-side-data/working-dir/cluster-resources/*-oc-mirror.yaml)
EOF
EOF
----
...
imageContentSources:
  - mirrors:
    - ip-10-0-57-47.ap-southeast-1.compute.internal:8443/openshift/release
    source: quay.io/openshift-release-dev/ocp-v4.0-art-dev
  - mirrors:
    - ip-10-0-57-47.ap-southeast-1.compute.internal:8443/openshift/release-images
    source: quay.io/openshift-release-dev/ocp-release
  - mirrors:
    - ip-10-0-57-47.ap-southeast-1.compute.internal:8443/web-terminal
    source: registry.redhat.io/web-terminal
  - mirrors:
    - ip-10-0-57-47.ap-southeast-1.compute.internal:8443/rhel8
    source: registry.redhat.io/rhel
----

These lines tell OpenShift that its installation images should be pulled from your `mirror-registry` instead of from quay.io
