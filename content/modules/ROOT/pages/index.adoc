...
The workshop environment (illustrated below) gives you access to several servers/systems in different networks. The instructions and environments were created using OpenShift version `{openshift_version}`.
...
* *Industrial / Utilities*: Use https://en.wikipedia.org/wiki/SCADA[SCADA] networks to protect their supervisory and control machinery
...
The lab instructions will help you prepare the [.lowside]#jump system# to download the installation content, configure the [.highside]#highside system# system to host the content, and create the [.highside]#openshift.disco.lab cluster# -- which doesn't exist yet.
