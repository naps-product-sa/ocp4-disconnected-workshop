...
1. Prepare a system ([.lowside]#jump#) that can download the mirroring tools and installation images
** Use `oc-mirror` to download the content
2. Setup a system ([.highside]#highside#) that can _serve the installation images_ in the disconnected network
** Use `oc-mirror` to populate a `mirror-registry`
3. Move the mirroring tools and installation images from the [.lowside]#jump system# to the [.highside]#highside system#
...
4. Tell the OpenShift installation program (`openshift-install`) where to find the installation images
...
[NOTE]
--
OpenShift 4.10 introduced `oc-mirror` as the new method to download OpenShift installation images.
The previous method, `oc adm release mirror ...`, is still available, but not recommended.
...
Now that the mirroring and installation tools have been downloaded and extacted, it's time to tell `oc-mirror` what to download. Let's start with a brief overview of using `oc-mirror`:

{counter:mirror}. Provide your access credentials (a **Pull Secret**)

** Credentials are required to download OpenShift installation images

{counter:mirror}. Create a YAML file that describes:

** What to download (OpenShift, an example Operator, an example image)
...
Please avoid making changes to the provided `ImageSetConfig` because it will increase the amount of time required to download and transfer the content.
--
...
[WARNING]
--
Please run the `oc-mirror` command in a `tmux` screen.
