...
On the Disco Cluster addtional components need to be added. This includes ClusterCatalog, CatalogSources, ImageDigestMirrorSets, ImageTagMirrorSets, etc. Which are located in the working directory generated by the oc-mirror command on the High Side Machine

[.highside,source,bash,role=execute]
----
oc apply -f /mnt/high-side-data/working-dir/cluster-resources/
----
